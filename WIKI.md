...
[[https://github.com/rubenvanwanzeele/loxberry-plugin-samsung-frame-tv/releases]]
...
- In **TV Configuration**, configure one or more TVs. For each TV, enter the IP address and optionally the MAC address. The MAC address is discovered automatically via ARP on save when possible.
- Mark one TV as the **primary TV**. That TV will also publish on the original base MQTT topics (backwards compatible with v1.0).
...
Each TV stores its own pairing token and reuses it automatically — you only need to pair each TV once.
...
| `loxberry/plugin/samsungframe/state/<device_id>` | `off` / `art` / `on` | retained, per device |
| `loxberry/plugin/samsungframe/state/availability/<device_id>` | `online` / `offline` | retained, per device availability |
...
The base topics and device IDs are configurable in the plugin web UI. Device topics are derived automatically by appending `/<device_id>`.

### Commands (Loxone → plugin)

| Topic | Target |
|---|---|
| `loxberry/plugin/samsungframe/cmd` | Primary TV |
| `loxberry/plugin/samsungframe/cmd/<device_id>` | One specific TV |
| `loxberry/plugin/samsungframe/cmd/all` | All enabled TVs |
...
2. Topic:
    - `loxberry/plugin/samsungframe/state` for the primary TV, or
    - `loxberry/plugin/samsungframe/state/<device_id>` for a specific TV
3. Enable **Retain**
...
**Only the primary TV responds on the base topic**
